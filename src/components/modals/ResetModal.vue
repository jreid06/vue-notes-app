      ...
      <div class="modal-content border border-danger bg-danger">
        ...
        <div class="modal-body">
			<template v-if="allCategories.length > 0">
				<p>
					This will delete &nbsp;<b class="h3 text-success">{{allCategories.length}}</b>&nbsp;
					{{allCategories.length ===1 ? 'category' : 'categories'}}?
				</p>
				<p>
					<span class="font-weight-bold">Note: </span> 
					This action will also delete &nbsp;<b class="h3 text-success">{{allNotes.length}}</b>&nbsp;
        ...
        <div class="modal-footer border-0">
          ...
          <button type="button" class="btn text-success" @click="confirmReset" v-if="allCategories.length > 0">
