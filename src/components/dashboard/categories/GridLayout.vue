      ...
      >
        <div class="card hfx-320">
          ...
          <div class="card-body">
            ...
            <hr>
            <div class="rounded border border-light bg-light shadow-sm p-2 d-flex flex-wrap hfx-135 oy-s ox-h">
              <template v-if="n.notes.length > 0">
                ...
                >
                  <!-- {{i}} -->
                  <router-link :to="`/dashboard/note/${n.notes[i].id}`" :style="{color: n.colour}" v-if="i < n.notes.length">
                     <i :class="getNote(n.notes[i].id).note.icon"></i>
                    <p class="h6"> <i class="fas fa-info-circle" data-toggle="tooltip" data-placement="bottom" :title="'Title: '+ getNote(n.notes[i].id).note.title"></i></p>
                  </router-link>
                  <p v-else class="pt-3 h3"> <i class="fas fa-plus-circle" @click="triggerModalLocal" data-action="note" :data-category-id="n.key" :style="{color: '#9b9b9b'}"></i>
                 </p>
                </div>
              </template>
              <template v-else>
