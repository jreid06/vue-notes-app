...
export default {
  ...
  methods: {
    ...
    triggerModal(e) {
      ...
      if (action === 'viewnote') {
        let itemID = target.hasAttribute('data-item-id') ? target.attributes['data-item-id'].value : 0,
          type = target.hasAttribute('data-item-type') ? target.attributes['data-item-type'].value : 0;


        if (type === 'note') {
          item = vm.getNote(itemID).note;
        }

        if (type === 'category') {
          item = vm.getCategory(itemID).cat;
        }
